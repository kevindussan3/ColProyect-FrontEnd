        ...
        <div class="card-body px-lg-5 py-lg-5">
          ...
          >
            ...
            <div class="custom-control custom-checkbox custom-control-alternative">
              <input
                v-model="usuario.recordar"
                id="vULJSPm"
                class="custom-control-input"
                type="checkbox"
              />
              <label for="vULJSPm" class="custom-control-label"
                ><span class="text-muted">Recordar</span></label
              >
            </div>
...
export default {
  data() {
    return {
      ...
      formFields: [
        {
          name: "email",
          placeholder: "Correo / Identificación",
          type: "text",
  ...
  methods: {
    ...
    async submitLogin() {
      try {
        ...
        console.log(this.recordar);
        if (this.recordar) {
          localStorage.setItem("usuarioRecordado", JSON.stringify(this.usuario));
        } else {
  ...
  created() {
    ...
    if (usuarioRecordado) {
      this.usuario = JSON.parse(usuarioRecordado);
