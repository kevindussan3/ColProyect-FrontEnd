        ...
        <template v-slot:default="row">
          <td>
            <div class="media-body">
              <span class="name mb-0 text-sm">{{ row.item.nombres }}</span>
            </div>
          </td>
          <td class="budget">
            {{ row.item.apellidos }}
          ...
          <td>
            <input type="text" size="4" class="narrow-font" >
          </td>
        </template>
      </base-table>
...
export default {
  ...
  methods: {
    ...
    async users() {
      try {
        const res = await fetch(this.url + `api/docente/estudiantes/${this.id}`, {
          headers: {
            "Content-Type": "application/json",
            "x-access-token": this.toke,
          },
